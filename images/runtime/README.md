...
推送稳定 SemVer `vMAJOR.MINOR.PATCH` Git tag 后，CI 会执行完整回归，并把通过
...
每次发布都会生成精确版本、`sha-...` 和 `latest` 标签。普通 `main` push、Pull Request
和手动 CI 只执行验证，不发布镜像。需要固定运行内容时，使用版本标签、SHA 标签或
镜像 Digest。当前只构建 `linux/amd64`。

```text
精确版本：ghcr.io/tencentcloudagentruntime/sandcamp-runtime:v0.1.0
浮动版本：ghcr.io/tencentcloudagentruntime/sandcamp-runtime:latest
```
...
需要可复现部署时，应同步精确版本、`sha-<完整 Git Commit>` 标签或固定 Digest，而
不是可变的 `latest` 标签。GHCR 包首次生成后，还需要仓库维护者在 Package Settings
中确认其读取可见性。
